...
export function groupServiceRows(rows: DbServiceRow[]): ServiceCategory[] {
  ...
  return [...map.values()].sort((a, b) => {
    const aOrder = rows.find((r) => r.service_categories?.slug === a.slug)?.service_categories
      ?.sort_order ?? 0;
    const bOrder = rows.find((r) => r.service_categories?.slug === b.slug)?.service_categories
      ?.sort_order ?? 0;
